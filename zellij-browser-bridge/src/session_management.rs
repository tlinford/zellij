...
/// Mirror of the client-side socket-path length guard. Inlined here so the
/// bridging crate has no dependency on `zellij-client`.
fn check_ipc_pipe_length(ipc_pipe: &std::path::Path) {
...
pub fn create_ipc_pipe(session_name: &str) -> PathBuf {
    ...
    check_ipc_pipe_length(&zellij_ipc_pipe);
    zellij_ipc_pipe
}
