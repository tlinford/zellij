use super::super::*;
use super::super::RemoteClientError;
...
mod tests {
    ...
    async fn call_attach_to_remote_session(
        ...
        tokio::task::spawn_blocking(move || {
            let runtime_owner = tokio::runtime::Builder::new_multi_thread()
                .enable_all()
                .build()
                .unwrap();
            let runtime = runtime_owner.handle().clone();
    ...
    async fn test_server_url_extraction() {
        ...
        let test_cases = vec![
            ...
            (
                "https://zellij.online/r/abc123#k=topsecret",
                "https://relay.zellij.online/r/abc123",
            ),
            (
                "https://zellij.online/r/abc123#k=s&r=relay.example.net",
                "https://relay.zellij.online/r/abc123",
            ),
            (
                "https://localhost:8443/r/wh3uwd96#k=s",
                "https://relay.localhost:8443/r/wh3uwd96",
            ),
        ];
    ...
    fn seed_mock_session(
        ...
        String,
        crate::attach::http_client::HttpClientWithCookies,
    ) {
        let session_token = uuid::Uuid::new_v4().to_string();
        let web_client_id = uuid::Uuid::new_v4().to_string();
        server_state
        ...
        let http_client =
            crate::attach::http_client::HttpClientWithCookies::new(None, true).unwrap();
...
mod tests {
    ...
    #[test]
    fn relay_join_url_derives_relay_subdomain() {
        let got = extract_server_url("https://zellij.online/r/abc123#k=topsecret").unwrap();
        assert_eq!(got, "https://relay.zellij.online/r/abc123");
    }

    #[test]
    fn relay_join_url_inherits_app_origin_port() {
        let got = extract_server_url("https://localhost:8443/r/abc#k=s").unwrap();
        assert_eq!(got, "https://relay.localhost:8443/r/abc");
    }

    #[test]
    fn relay_join_url_ignores_relay_override() {
        let got =
            extract_server_url("https://zellij.online/r/abc123#k=topsecret&r=relay.example.net")
                .unwrap();
        assert_eq!(got, "https://relay.zellij.online/r/abc123");
    }

    #[test]
    fn relay_join_url_ip_host_passthrough() {
        let got = extract_server_url("http://127.0.0.1:8765/r/abc#k=s").unwrap();
        assert_eq!(got, "http://127.0.0.1:8765/r/abc");
    }

    #[test]
    fn relay_join_url_does_not_double_prefix() {
        let got = extract_server_url("https://relay.zellij.online/r/slug").unwrap();
        assert_eq!(got, "https://relay.zellij.online/r/slug");
    }

    #[test]
    fn join_secret_extracted_from_fragment() {
        assert_eq!(
            join_secret_from_url("https://zellij.online/r/abc#k=topsecret&r=h"),
            Some("topsecret".to_string())
        );
        assert_eq!(join_secret_from_url("https://zellij.online/r/abc"), None);
        assert_eq!(join_secret_from_url("https://example.com/session"), None);
    }

    #[test]
    fn derive_relay_host_rule() {
        assert_eq!(derive_relay_host("zellij.online"), "relay.zellij.online");
        assert_eq!(
            derive_relay_host("relay.zellij.online"),
            "relay.zellij.online"
        );
        assert_eq!(derive_relay_host("127.0.0.1"), "127.0.0.1");
        assert_eq!(derive_relay_host("[::1]"), "[::1]");
    }
}
